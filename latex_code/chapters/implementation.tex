...
\todo[inline]{Describe \textbf{SSA} problem and fix as soon as tests work}
\todo[inline]{In case we do anything about the enforcement of the programming model, describe here. Maybe move the comment about adding tests hee also.}
\subsection{Extending the supported Syntax}
\todo[inline]{Not sure if this will remain/become s section. Maybe merge}
\todo[inline]{would be cool if I got `while` transformation running}
...
Now we represent tuple expressions as lists of literals or variables \code{Tuple [Either Lit Var]} and indexing via the more general term \code{Indexing bnd num}, representing indexing of \code{bnd} at the natural number index \code{num}. \\

\todo[inline]{Sebastians tuple type propagation fix. Was? Warum? Wie?}
...
\todo[inline]{Chekout Rust AST lowering in the rustc repo}
...
Although it is slightly counter intuitive as Ohuas Frontend Language is functional and functional languages are notorious for pattern matching, there is no syntax for Rusts \code{match} expressions in Ohuas frontend language. So currently we either need to transform it into more basic constructs i.e. i-t-e, or encapsulate it in a library function. \\
...
let x = match some_call() {
 ...
 let (x, *variables used after match*)  = 
    match_fun(*variables from scope used in the match*);
 //----------------
...
However this approach is not as trivial as it looks at first glance. It requires knowledge about the enclosed variables from the surrounding scope and encapsulation also changes the semantics of returning values or errors.. Branches returning by a no-semicolon statement need to be rewritten to use \rust{return}, branches with explicit \rust{return}, \rust{unwrap} or \rust{?} must lead to conditional returns in the outer/original scope of the match.\\

 \todo[inline]{Instead we can see match expression as what they really are ... each pattern actually is a function from the matches value to bool, repeated matches are if match1 else if match2 else if .... So we can a) turn each match into a bool function(see below) and then transform the whole thing to nested if-else.}

 
 \todo[inline]{Actually more efficient and also much cooler than nesting if else is supporting multi-case switch directly. by turning our 'branching-switches' from a ((0/1), (True/False)) system to a (num\_cases, num) semantic.  
 This also happens basically when the rust compiler lowers either if-else or matches (see ref below). If we have an enumerated switch, the more efficent way to transfom the match is to have one match function, that return the index of the matched expression. We could encapsulate it in a library but due to import/visibility issues concerning the patterns it would actually be niver to have a way to encapsulate code 'in place' i.e. Ohua treads it as a function, but at least in the backend it's just inlined again.}
\href{https://rustc-dev-guide.rust-lang.org/mir/construction.html#lowering-expressions-into-the-desired-mir}{rustc docu}
...
\todo[inline]{Example using SSA and state threading replacement ... is this sufficient?}
